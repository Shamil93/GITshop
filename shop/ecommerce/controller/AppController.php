...
require_once( "ecommerce/command/DefaultCommand.php" );
...
//        echo "<tt><pre>".print_r($this->controllerMap, true)."</pre></tt>";
        if( ! self::$base_cmd ) {
            self::$base_cmd = new \ReflectionClass( "\\ecommerce\\command\\Command" );   // абстрактный класс Command
            self::$default_cmd = new \ecommerce\command\DefaultCommand();    // класс по умолчанию (будет вызываться, если нет параметров)
        }
    }
     ...
     * @param Request $req
     * @return \ecommerce\command\DefaultCommand|null|object
    ...
    function getCommand( Request $req ) {
        ...
        if( ! $previous ) { // если не было - т.е. приложение запущено в первый раз
            ...
            if( ! $cmd ) {  // если команды нет
                ...
                return self::$default_cmd;  // возвращаем класс по умолчанию - new \ecommerce\command\DefaultCommand();
            }
        ...
        if( ! $cmd_obj ) {  // если команда не известная
            throw new \ecommerce\base\AppException( "could not resolve '$cmd''" );
        }
        ...
        if( isset( $this->invoked[$cmd_class] ) ) { // если флаг выставлен, то приложение вызывается по кругу
            throw new \ecommerce\base\AppException( "circular forwarding" );
        }
    ...
    function resolveCommand( $cmd ) {
        ...
        $filepath = "ecommerce/command/$classroot.php";  // путь до скрипта - он же имя команды
        $classname = "\\ecommerce\\command\\$classroot"; // в скрипте должен быть класс такого же названия
