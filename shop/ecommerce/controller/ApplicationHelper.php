...
require_once( "ecommerce/controller/AppController.php" );
...
class ApplicationHelper {
    ...
    private $config = "ecommerce/data/ecommerce_options.xml"; // путь до файла конфигурации
     ...
     */
    function init() {
        $dsn = \ecommerce\base\ApplicationRegistry::getDSN();    // пытаемся получить кэшированный дескриптор базы данных
        if( ! is_null( $dsn ) ) {
            return;
        }
    ...
    private function getOptions() {
        ...
        \ecommerce\base\ApplicationRegistry::setDSN( $dsn ); // кэшируем дескриптор базы данных
        ...
        foreach( $options->control->view as $default_view ) {   // находим вьюшки по умолчанию <view>
            ...
            $status = \ecommerce\command\Command::statuses( $stat_str ); // метод statuses командного класса сохраняет их цифровой эквивалент
//            'CMD_DEFAULT'           => 0,
//            'CMD_OK'                => 1,
//            'CMD_ERROR'             => 2,
        ...
        foreach ( $options->control->command as $command_view ) {   // находим все команды <command>
            ...
            if( $command_view->view ) { // находим в теле команды вьюшки
                ...
                foreach ( $command_view->status as $command_view_status ) { // находим в теле комманд статус <status>
                    ...
                    $status = \ecommerce\command\Command::statuses( $stat_str ); // получаем цифровой эквивалент статуса
                    if( $view ) {
...
//        echo "<tt><pre>".print_r($map, true)."</pre></tt>";
        \ecommerce\base\ApplicationRegistry::setControllerMap( $map );   // кэшируем полученный класс в файл в Registry
    }
    ...
    private function ensure( $stmt, $msg ) {
        if( ! $stmt ) {
            throw new \ecommerce\base\AppException( $msg );
        }
    }
}
?>