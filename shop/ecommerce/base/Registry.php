...
class RequestRegistry extends Registry {
    ...
    /**
     * Статический метод для вызова защищенного метода
     * @param \ecommerce\controller\Request $request
     */
    static function setRequest( \ecommerce\controller\Request $request ) {
...
class ApplicationRegistry extends Registry {
    private static $instance;
    private $freeezedir = "ecommerce/data";
    private $values = array();
     ...
     */
    protected function get( $key ) {
        $path = $this->freeezedir . DIRECTORY_SEPARATOR . $key; // "ecommerce/data"."/"."dsn"
     ...
     * @param \ecommerce\controller\ControllerMap $map
     */
    static function setControllerMap( \ecommerce\controller\ControllerMap $map ) {
        self::instance()->set( 'cmap', $map );
    }

    /**
     * Статичный метод, чтобы вернуть карту приложения и методы класса AppController
     * @return \ecommerce\controller\AppController
     */
    static function appController() {
        ...
        if( ! isset( $obj->appController ) ) {  // если объект не существует
            ...
            $obj->appController = new \ecommerce\controller\AppController( $cmap );  // создаем экземпляр AppController и передаем в него карту приложения
        }
