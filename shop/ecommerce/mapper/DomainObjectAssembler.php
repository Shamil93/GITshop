...
//        echo "<tt><pre>".print_r($factory, true)."</pre></tt>";
        ...
        if( ! isset( self::$PDO ) ) { // если еще нет
            $dsn = \ecommerce\base\DBRegistry::getDB(); // то сохраняем дескриптор БД
            if( is_null( $dsn ) ) { // если неудача
                throw new \ecommerce\base\AppException( "No DSN" ); // вызываем исключение
            }
...
//        echo "<tt><pre>".print_r($values, true)."</pre></tt>";
    ...
    function insert( \ecommerce\domain\DomainObject $obj ) {
...
//        return $pagfact->getPage();
        return array ( "navigation"=>$pagfact->printPageProducts(), "select"=>$this->factory->getCollection( $pagfact->getPage() ) );
